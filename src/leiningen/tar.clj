...
(defn- entry [f n fmt]
       (case fmt
             (:tgz :tar-gz) (doto (TarEntry. f)
                                  (.setName n)
                                  #(when (.canExecute f)
                                         (.setMode % 0755)))
             (:zip) (doto (ZipEntry. f n)
                          #(when (.canExecute f)
                                 (.setUnixMode % 0755)))
             (doto (TarEntry. f)
                   (.setName n)
...
(defn- add-directory [tar path fmt]
       ;; minor hack, we use the cwd as the model for any plain directories
       ;; that we're adding
       (let [entry (doto (entry (io/file cwd) path fmt))]
            (.putNextEntry tar entry)
            (.closeEntry tar)))
...
(defn add-jars [project tar path jar fmt]
      (let [options (:tar project)]
           (add-directory tar (str path "/lib") fmt)
           (add-file tar (str path "/lib") (io/file jar) fmt)
           (if-not (:uberjar options)
                   (doseq [j (jars-for project)]
                          (add-file tar (str path "/lib") j fmt)))))
...
(defn- out-stream [fmt tar-file]
       (let [file-stream (FileOutputStream. tar-file)]
            (case fmt
                  ...
                  (:zip) (ZipOutputStream. file-stream)
                  file-stream)))
...
(defn tar [project & args]
      ...
      (let [options (:tar project)
           ...
           (let [ostream (out-stream fmt tar-file)] (with-open [tar (if (= fmt :zip)
                                                                      ostream
                                                                      (doto (TarOutputStream. ostream)
                                                                            (.setLongFileMode tar TarOutputStream/LONGFILE_GNU)))]
                                                               ;; and add everything from pkg
                                                               (doseq [p (file-seq (io/file (:root project) "pkg"))]
                                                                      (add-file tar tar-path p fmt))
                                                               ;; and whatever jars should be included
                                                               (add-jars project tar tar-path jar fmt)
